...
def solve_optimization(
    ...
    pipe_os_3,       # Arriving at T+3 (from past ?) - Usually logic suggests OS pipeline implies T+3 if user inputs it.
                     # Prompt says "Overseas arriving in: 1 week, 2 weeks, 3 weeks".
                     # If we order now (T), it arrives at T+3. 
                     # If there is ALREADY something arriving in 3 weeks, it stacks.
    scenarios,
    horizon,
    local_enabled,
    overseas_enabled
    ...
    # These are the recommendations we want.
    ...
    # Check T=0 constraints based on initial state (which is common to all scenarios)
    # If initial state is disrupted, force Overseas_0 = 0
    if not scenarios[0]['overseas_available'][0] or not overseas_enabled:
         prob += overseas_order_0 == 0
    
    # Variables per scenario (Wait-and-See for T > 0)
    # Stored in dicts: keys (t, s)
    inv_vars = {}
    back_vars = {}
    local_vars = {}
    overseas_vars = {}
    ...
    for s_idx, scenario in enumerate(scenarios):
        ...
        for t in range(horizon):
            ...
            # Logic:
            # T=0: 0 (Already in prev_inv_expr)
            # T=1: Pipe_Loc + Pipe_Os1 + Order_Loc(T=0)
            # T=2: Pipe_Os2 + Order_Loc(T=1)
            # T=3: Pipe_Os3 + Order_Loc(T=2) + Order_Os(T=0)
            # T>=4: Order_Loc(T-1) + Order_Os(T-3)
            ...
            # Balance Constraint
            # Ends_Inv - Ends_Back = Starts_Inv - Starts_Back + Arrivals - Demand
            # Ends = Starts + Arrivals - Demand
            # If t=0, Starts = prev_inv_expr
            # If t>0, Starts = inv_{t-1} - back_{t-1}
    ...
    # 1. Backorder Prob at T=1 (Next week)
    # We look at back_vars[(0, s)] which is end of this week T=0.
    # Wait, "Probability of backorder next week".
    # Does that mean end of T=0 or end of T=1?
    # "Next week" implies T+1.
    # Let's start with T=0 (This week's outcome) and T=1 (Next week's outcome).
    # App usually shows what happens *after* the current decision.
    # So "End of Next Week" (T=1) is useful.
    
    sim_data = [] # List of dicts for DataFrame
    scenario_costs = []
    
    for s in range(N):
        # Re-calculate cost for this scenario to be precise
        # (Though we have expression, value(scenario_cost) might work if we kept reference)
        # We'll just sum variable values.
        c = 0
    ...
    # Metrics
    # Prob Backorder Next Week (T=1?? Prompt says "Backorder probability next week". Usually means T=0 end state or T=1?)
    # "Inputs... Week T". "Recommendations for THIS week (T)". 
    # "Next week" usually refers to T+1.
    # Let's calculate for T=0 (Immediate outcome) and T=1.
    # We'll display T=1 as requested "Next week".
...
st.markdown("""
**Goal:** Minimize expected costs (Orders + Holding + Backlog).
This tool optimizes ordering decisions for the current week using scenario-based stochastic programming.
""")

with st.sidebar:
    st.header("1. Game Parameters")
    horizon = st.number_input("Planning Horizon (Weeks)", 4, 12, 8)
    ...
    on_hand = col1.number_input("On-hand Inventory (Start)", 0, 10000, 1000)
    backlog = col2.number_input("Backorders (Start)", 0, 10000, 0)
    
    st.subheader("Pipeline (Incoming)")
    st.caption("Enter quantities ALREADY ordered/arriving.")
    pipe_local = st.number_input("Local Arriving Next Week (T+1)", 0, 5000, 0)
    pipe_os1 = st.number_input("Overseas Arriving in 1 wk (T+1)", 0, 5000, 0)
    pipe_os2 = st.number_input("Overseas Arriving in 2 wks (T+2)", 0, 5000, 0)
    pipe_os3 = st.number_input("Overseas Arriving in 3 wks (T+3)", 0, 5000, 0)
    
    st.subheader("Disruption Status")
    disruption_rem = st.slider("Disruption Remaining Weeks", 0, 3, 0, help="0 = Not disrupted. 1-3 = Active.")
...
if 'res' in st.session_state:
    ...
    with col_rec1:
        st.info("**Local Order** (Cost 6.5)")
        st.metric("Quantity", f"{res['local_order']:,.0f}", delta=None)
        
    with col_rec2:
        st.info("**Overseas Order** (Cost 5.0)")
        st.metric("Quantity", f"{res['overseas_order']:,.0f}", delta=None)
    
    # --- Risk Analysis ---
    st.markdown("### 📊 Predicted Outcomes (End of Week T+1)")
    r1, r2, r3 = st.columns(3)
    r1.metric("Backorder Probability", f"{res['prob_backorder_t1']*100:.1f}%")
    r2.metric("Expected Net Inventory", f"{res['expected_inv_t1']:,.0f}")
    r3.metric("Avg Scenario Cost", f"${res['df_costs']['Cost'].mean():,.0f}")
    ...
    with tab1:
        ...
        st.caption("Shaded area: 10th-90th percentile range. Line: Median.")
